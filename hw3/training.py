        ...
        for epoch in range(num_epochs):
            ...
            if early_stopping is not None:
                if (best_acc is None) or (test_result.accuracy >= best_acc):
                    ...
                    epochs_without_improvement = 0
                    save_checkpoint = True  # save model checkpoint if requested at the next block of code
                else:
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # to delete:
        # Reset the gradients to zero using self.optimizer.zero_grad()
        # Compute the gradients of the loss with respect to the model’s parameters using loss.backward()
        # Update the model’s parameters using the computed gradients and the chosen optimization algorithm with self.optimizer.step()

        #  Forward pass
        out, self.hidden_state = self.model(x, self.hidden_state)
        out = out.transpose(1, 2)  # (B,S,V) -> (B,V,S)
...
class VAETrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        z, mu, log_sigma2 = self.model(x)
        self.optimizer.zero_grad()
        loss, data_loss, kl_loss = self.loss_fn(x, z, mu, log_sigma2)
        loss.backward()
        self.optimizer.step()
        # ========================
...
class FineTuningTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        num_correct = torch.sum(y_pred == labels).detach().item()
