        ...
        for i in range(len(channels)-1):
            modules.extend([
                nn.Conv2d(in_channels=channels[i],
                          ...
                          stride=(2, 2),
                          padding=(2, 2)),
                nn.BatchNorm2d(num_features=channels[i+1], momentum=0.9,  eps=1e-6),
                nn.ReLU()
            ])
...
class DecoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        for i in range(len(channels) - 1):
            modules.extend([
                nn.BatchNorm2d(num_features=channels[i], momentum=0.9, eps=1e-6),
                nn.ReLU(),
...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        # ====== YOUR CODE: ======
        self.fc_mu = nn.Linear(in_features=n_features, out_features=z_dim)
        self.fc_log_sigma2 = nn.Linear(in_features=n_features, out_features=z_dim)
        self.fc_z = nn.Linear(in_features=z_dim, out_features=n_features)
    ...
    def encode(self, x):
        ...
        z = mu + torch.sqrt(sigma2) * torch.randn_like(sigma2, device=device)
        # ========================
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    # ====== YOUR CODE: ======
    # Calculate data_loss
    norm_diff = torch.norm(x - xr, 2)
    squared_norm_diff = norm_diff ** 2
    num_elements = x.view(-1, 1).shape[0]
    data_loss = squared_norm_diff / (x_sigma2 * num_elements)

    # Calculate kldiv_loss
    exp_z_log_sigma2 = torch.sum(z_log_sigma2.exp())
    squared_norm_z_mu = torch.norm(z_mu, 2) ** 2
    sum_z_log_sigma2 = z_log_sigma2.sum()

    kldiv_numerator = exp_z_log_sigma2 + squared_norm_z_mu - sum_z_log_sigma2
    kldiv_denominator = z_mu.shape[0]
    kldiv_loss = kldiv_numerator / kldiv_denominator - z_mu.shape[1]
